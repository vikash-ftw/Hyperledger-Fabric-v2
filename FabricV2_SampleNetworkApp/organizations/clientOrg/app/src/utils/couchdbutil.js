...
const createDatabaseIfNotExists = function (nano, dbname) {
...
const writeToCouchDB = async function (nano, dbname, key, value) {
    ...
    return new Promise((async (resolve, reject) => {
        ...
        try {
            ...
            console.error(`**-- COUCHDB DB Creation Error: ${error} --**`)
        }
        ...
        if (key == null) {
            ...
            db.get(key, async function (err, body) {
                ...
                db.insert(updateValue, key, async function (err, body, header) {
                    if (err) {
                        console.error(`**-- COUCHDB Insert Error: ${err} --**`);
                        reject(err);
                    }
                });
            });
...
const deleteRecord = async function (nano, dbname, key) {
        ...
        db.get(key, async function (err, body) {
            ...
            if (err == null) {
                ...
                db.destroy(key, revision, async function (err, body, header) {
                    if (err) {
                        console.error(`**-- COUCHDB Delete Error: ${err} --**`);
                        reject(err);
                    }
                });
...
export default{createDatabaseIfNotExists, writeToCouchDB, deleteRecord};